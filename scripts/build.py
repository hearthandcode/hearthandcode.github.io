...
for name in ['style.css','app.js','dashboard-extras.css']:shutil.copy2(root/'web'/name,out/name)
...
index=(root/'web/index.html').read_text().replace('<!--COLLECTION-->','<div class="collection">'+rows+'</div>').replace('<!--CORPUS_SUMMARY-->',f'{len(articles)} practical field notes. Each with a formulation, a working example, and a counterexample.').replace('</head>','<link rel="alternate" type="application/rss+xml" title="Fieldcraft" href="feed.xml"></head>').replace('Back to top ↑','Back to top ↑').replace('https://github.com/hearthandcode/fieldcraft','https://github.com/hearthandcode/hearthandcode.github.io')
