...
for name in ['style.css','app.js','dashboard-extras.css','catalog-popup.css','catalog.js','catalog-lab.css','catalog-lab.js']:shutil.copy2(root/'web'/name,out/name)
for name in ['examples','fixtures','content']:(out/name).mkdir(exist_ok=True)
for a in articles:
...
catalog_path=root/'web/catalog-entries.json'
catalog_entries=json.loads(catalog_path.read_text()) if catalog_path.exists() else []
catalog_templates=root/'web/catalog-templates.json'
def e(value):return html.escape(str(value),quote=True)
def md(text):return markdown.markdown(text,extensions=['fenced_code','tables']).strip()
def demo(a):
...
for index,a in enumerate(ordered):
 ...
 refs=[];raw_refs=iter(a.get('source_refs',[]));pending=list(raw_refs);ref_index=0
 while ref_index<len(pending):
  ...
  elif isinstance(ref,str) and ref.startswith('https://'):url=ref;parts=urlsplit(url);title=parts.netloc+' · '+(parts.fragment or parts.path.rstrip('/').split('/')[-1] or 'Reference')
  elif ref_index<len(pending) and isinstance(pending[ref_index],str) and pending[ref_index].startswith('https://'):title=ref;url=pending[ref_index];ref_index+=1
  else:url='';title=ref
...
index=(root/'web/index.html').read_text().replace('<!--COLLECTION-->','<div class="collection">'+rows+'</div>').replace('<!--CORPUS_SUMMARY-->',f'{len(articles)} practical field notes. Each with a formulation, a working example, and a counterexample.').replace('</head>','<link rel="alternate" type="application/rss+xml" title="Fieldcraft" href="feed.xml"></head>').replace('https://github.com/hearthandcode/fieldcraft','https://github.com/hearthandcode/hearthandcode.github.io')
...
if catalog_entries:
 ...
 grid_html=''
 for cat_entry in catalog_entries:
  grid_html+=f'<div class="catalog-card" data-slug="{e(cat_entry["slug"])}" data-type="{e(cat_entry["type"])}">'
  grid_html+=f'<span class="card-type">{e(type_labels.get(cat_entry["type"],cat_entry["type"]))}</span>'
  grid_html+=f'<span class="card-number">#{cat_entry["number"]}</span>'
  grid_html+=f'<h3>{e(cat_entry["title"])}</h3></div>'
 entries_json=json.dumps([{'slug':ce['slug'],'title':ce['title'],'type':ce['type'],'number':ce['number'],'sections':[{'title':s['title'],'body_html':md(s['body'])} for s in ce['sections']]} for ce in catalog_entries],ensure_ascii=False)
 catalog_html=f'''<section class="catalog-header"><p class="eyebrow">128 RETAINED METHODS</p><h2>Prompt Engineering Catalog</h2><p class="deck">Each technique is organized by what it does to information, not what topic it addresses. Click any card to open its full details: how it works, when to use it, and what it cannot do.</p></section><div class="catalog-filters">{filter_html}</div><input id="catalog-search" type="search" placeholder="Search techniques by name or description…" style="display:block;width:calc(100% - 12%);max-width:1000px;margin:0 auto 1.5rem;padding:10px 16px;border:1px solid var(--rule);border-radius:8px;background:var(--panel);color:var(--ink);font-size:14px"><div class="catalog-grid">{grid_html}</div><div class="catalog-overlay" id="catalog-overlay"><div class="catalog-popup" role="dialog" aria-label="Technique details"><div class="catalog-popup-header"><span class="popup-type" id="catalog-popup-type"></span><span class="popup-number" id="catalog-popup-number"></span><button class="catalog-popup-close" id="catalog-popup-close" aria-label="Close">&times;</button><h2 id="catalog-popup-title"></h2></div><div class="catalog-popup-body" id="catalog-popup-body"></div></div></div><script id="catalog-data" type="application/json">{entries_json}</script>'''

# --- Detail pages with catalog injection ---
def render_section(section):
 return f'<section><h2>{e(section["title"])}</h2>{md(section["body"])}</section>'

for page in detail_pages:
 sections=''.join(render_section(section) for section in page['sections'])
 ...
 page_html=f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="{e(page['deck'])}"><title>{e(page['title'])} · Hearth &amp; Code</title><link rel="stylesheet" href="style.css"><link rel="stylesheet" href="dashboard-extras.css"></head><body class="detail-page"><a class="skip" href="#main">Skip to content</a>{header}<main id="main"><section class="detail-hero"><div><p class="eyebrow">{e(page['number'])} / {e(page['kicker'])}</p><h1>{e(page['title'])}</h1><p class="detail-deck">{e(page['deck'])}</p><p class="detail-summary">{e(page['summary'])}</p></div>{visual}</section><div class="detail-layout"><aside class="detail-aside"><a href="./">← Orientation</a><p class="eyebrow">EXPLORE THE LAYER</p>{detail_nav}<a class="fieldcraft-link" href="fieldcraft.html">Fieldcraft library ↗</a></aside><article class="detail-article">{sections}<section class="detail-limit"><p class="eyebrow">KEEP THE CLAIM BOUNDED</p>{md(page['limit'])}</section></article></div></main><footer><span>HEARTH &amp; CODE · PUBLIC ORIENTATION</span><span>SOURCE-AWARE / BOUNDED</span><a href="./">Back to orientation ↑</a></footer></body></html>'''
 # Inject catalog into prompt-catalog page
 if page['slug']=='prompt-catalog' and catalog_html:
  page_html=page_html.replace('</head>','<link rel="stylesheet" href="catalog-popup.css"><link rel="stylesheet" href="catalog-lab.css"><script type="module" src="catalog.js"></script><script type="module" src="catalog-lab.js"></script></head>')
  # Inject catalog grid before the claim-boundary section
  page_html=page_html.replace('<section class="detail-limit">',catalog_html+'<section class="detail-limit">')
 (out/f'{page["slug"]}.html').write_text(page_html)
if catalog_templates.exists():shutil.copy2(catalog_templates,out/'catalog-templates.json')
...
if catalog_entries:print(f'Rendered {len(catalog_entries)} catalog entries')